...
def run_simulation(
    ...
    # for all gamblers, washing out demographic signal in harm metrics.
    #
    # Demographic adjustments to log(F):
    #
    # Age: Braverman & Shaffer (2012, J. Gambling Studies) and Gainsbury et al.
    # (2015) document that younger gamblers display higher betting intensity and
    # more impulsive wagering patterns. We apply a declining age adjustment:
    # -0.015 per year above 25, capped. A 20-year-old bets ~35% more of DI
    # than a 50-year-old conditional on gambling.
    #
    # Income: Brosowski et al. (2019) [C5] and NBER w33108 show that lower-
    # income households spend a higher *fraction* of disposable income on
    # gambling (even if the absolute dollar amount is lower). This is consistent
    # with Engel's Law applied to leisure: necessities crowd out entertainment
    # less at higher incomes. Elasticity of -0.12 on log(income/median) means
    # a household at $30k bets ~20% more of DI than one at $150k.
    #
    # Gender: Male gamblers show higher intensity conditional on participation
    ...
    log_f  = rng.normal(mu_f_i, sigma_f, size=n)
    ...
    # Bet-level outcome variance:
    # Annual variance is sub-linear in wager count. Using sqrt(wagers) × scale
    # rather than wagers × 0.5 better reflects the law-of-large-numbers
    # attenuation for frequent bettors while preserving meaningful single-year
    # variance for casual bettors (Croson & Sundali 2005, J. Risk & Uncertainty).
    wager_sd      = np.sqrt(np.maximum(wagers, 1)) * 8.0
    outcome_noise = rng.normal(0, wager_sd, size=n)
...
def predict_individual(
    ...
    # Apply same demographic adjustment as population simulation
    age_adj_f    = -0.015 * max(age - 25, 0)
    income_adj_f = -0.12  * np.log(income / 75_000)
    gender_adj_f =  0.20  * male
    mu_f_i = mu_f + age_adj_f + income_adj_f + gender_adj_f

    gambles  = rng.random(n_simulations) < p
    log_f    = rng.normal(mu_f_i, SIGMA_F, n_simulations)
    f        = np.clip(np.exp(log_f), 0, 5.0)
    wagers   = di * f * gambles
    losses   = house_edge * wagers
    wager_sd = np.sqrt(np.maximum(wagers, 1)) * 8.0
    noise    = rng.normal(0, wager_sd, n_simulations)
...
def sensitivity_analysis(base_pop: pd.DataFrame) -> pd.DataFrame:
    ...
    for name, params in scenarios.items():
        ...
        # Apply same demographic adjustment as main simulation
        age_adj_f    = -0.015 * np.maximum(pop["age"].values - 25, 0)
        income_adj_f = -0.12  * np.log(pop["income"].values / 75_000)
        gender_adj_f =  0.20  * pop["male"].values
        mu_f_i = mu_f + age_adj_f + income_adj_f + gender_adj_f

        gamble   = (rng.random(len(pop)) < p_g).astype(int)
        f        = np.clip(np.exp(rng.normal(mu_f_i, params["sigma_f"], len(pop))), 0, 5)
        wagers   = di * f * gamble
        losses   = params["house_edge"] * wagers
        wager_sd = np.sqrt(np.maximum(wagers, 1)) * 8.0
        noise    = rng.normal(0, wager_sd, len(pop))
        net_loss = losses + noise
        ...
        results.append({
            ...
            "% Lose >5% DI":     f"{(nl_g > 0.05*di_g).mean()*100:.1f}",
            "% Lose >20% DI":    f"{(nl_g > 0.20*di_g).mean()*100:.1f}",
...
def plot_results(pop: pd.DataFrame, save_prefix: str = "q2"):
    ...
    # ── Figure 1: 4-panel overview (improved)
    ...
    ax = axes[0, 0]
    pos_losses = gamblers["net_loss"][gamblers["net_loss"] > 0]
    ax.hist(pos_losses, bins=np.logspace(0, 5, 60),
            color="steelblue", edgecolor="white", alpha=0.85)
    ax.set_xscale("log")
    ax.axvline(pos_losses.median(), color="orange", linewidth=1.5,
    ...
    ax = axes[1, 0]
    pct_by_income = (gamblers.groupby("income_tier", observed=True)["loss_pct_di"]
                     .mean())
    pct_by_income.plot(kind="bar", ax=ax, color="mediumseagreen",
                       edgecolor="white", width=0.7)
    ax.set_title("Mean Loss as % of DI by Income Tier  [C5][C9]"); ax.set_xlabel("")
    ax.set_ylabel("Mean Loss / DI (%)")
    ax.yaxis.set_major_formatter(mticker.FuncFormatter(lambda x, _: f"{x:.1f}%"))
    ax.tick_params(axis="x", rotation=20)

    # 1d: Loss as % of DI with dual thresholds [C9]
    ax = axes[1, 1]
    pct = gamblers["loss_pct_di"].clip(-10, 60)
    ...
    # ── Figure 1b: Absolute loss by income tier (separate, for comparison)
    fig, axes = plt.subplots(1, 2, figsize=(13, 5))
    fig.suptitle("Loss in Absolute Dollars vs. Share of Disposable Income  [C5][C9]",
    ...
    ax = axes[1]
    pct_by_income.plot(kind="bar", ax=ax, color="coral", edgecolor="white", width=0.7)
    ax.set_title("Mean Loss as % of Disposable Income  [C5]"); ax.set_xlabel("")
    ax.set_ylabel("Mean Loss / DI (%)")
    ax.yaxis.set_major_formatter(mticker.FuncFormatter(lambda x, _: f"{x:.1f}%"))
    ax.tick_params(axis="x", rotation=20)
    ax.annotate("Lower income -> greater proportional harm (Engel's Law [C5][C9])",
                xy=(0.5, 0.85), xycoords="axes fraction", ha="center",
                fontsize=9, color="gray", style="italic")

    plt.tight_layout()
    plt.savefig(f"{save_prefix}_income_harm.png", dpi=150, bbox_inches="tight")
    plt.close()
    print(f"Saved: {save_prefix}_income_harm.png")
